...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16, pool="max", substract=True):
        super().__init__()
        self.pool = pool
...
class PointTransformerBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16, pool="max", substract=True):
        super(PointTransformerBlock, self).__init__()
        self.linear1 = nn.Linear(in_planes, planes, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.transformer2 = PointTransformerLayer(planes, planes, share_planes, nsample, pool=pool, substract=substract)
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=6, k=13, pool="max", substract=True):
        super().__init__()
        self.c = c
        self.pool = pool
        self.substract = substract
    ...
    def _make_enc(self, block, planes, blocks, share_planes=8, stride=1, nsample=16):
        ...
        for _ in range(1, blocks):
            layers.append(block(self.in_planes, self.in_planes, share_planes, nsample=nsample, pool=self.pool, substract=self.substract))
...
def pointtransformer_pool_maxsub(**kwargs):
    model = PointTransformerSeg(PointTransformerBlock, [2, 3, 4, 6, 3], pool="max", substract=True, **kwargs)
    return model

def pointtransformer_pool_avgsub(**kwargs):
    model = PointTransformerSeg(PointTransformerBlock, [2, 3, 4, 6, 3], pool="avg", substract=True, **kwargs)
    return model

def pointtransformer_pool_max(**kwargs):
    model = PointTransformerSeg(PointTransformerBlock, [2, 3, 4, 6, 3], pool="max", substract=False, **kwargs)
    return model
